...
def get_salary_settings():
    ...
    break_start/break_end and pay_start/pay_end are stored as "HH:MM" strings (24hr).
    None means not set.
    """
    conn = get_connection()
    cursor = conn.cursor()
...
# Subtracts break time and enforces pay window.
# =====================================================
def to_time_str(val):
    """
    MySQL TIME columns come back as datetime.timedelta when using pymysql.
    Convert to 'HH:MM' string regardless of input type.
...
def compute_paid_minutes(time_in, time_out, settings):
    """
    Given time_in (datetime), time_out (datetime), and settings dict,
    returns the number of minutes that should be paid.

    Rules:
    1. Clamp time_in and time_out to the pay window (pay_start..pay_end) if set.
    2. Subtract any overlap with the break window (break_start..break_end) if set.
    """
    if time_out is None:
        time_out = datetime.now()

    # --- 1. Enforce pay window ---
    ...
    if break_start_str and break_end_str:
        ...
        # Overlap of [effective_in, effective_out] with [break_start, break_end]
...
# =====================================================
# NEW: UPDATE ALL SALARY SETTINGS (break + pay range)
# =====================================================
...
def update_salary_settings():
    ...
    Accepts JSON with any combination of:
      salary_per_minute, break_start, break_end, pay_start, pay_end
    All time values as "HH:MM" 24hr strings. Pass empty string "" to clear.
    """
    data = request.json
...
# =====================================================
# SCAN RFID (updated to use compute_paid_minutes)
# =====================================================
...
def scan():
    ...
    cursor.execute("""
        SELECT * FROM opti WHERE UPPER(REPLACE(rfid,' ',''))=%s
    """, (uid,))
    ...
    if not record:
        ...
        cursor.execute("""
            SELECT id FROM opti_rec WHERE id_employee=%s AND time_in=%s
        """, (employee["id_employee"], now))
...
# =====================================================
# NEW FEATURE 3: MANUAL TIME IN/OUT (admin for lost card)
# =====================================================
@app.route("/api/manual_time_in", methods=["POST"])
def manual_time_in():
    """Admin manually sets time in for an employee (e.g. lost card)."""
    if "admin" not in session:
        return jsonify({"status": "error", "message": "Unauthorized"}), 401

    data = request.json
    emp_id = int(data.get("employee_id"))
    time_in_str = data.get("time_in")  # "HH:MM" 24hr
...
def manual_time_out():
    """Admin manually sets time out for an employee (e.g. forgot to sign out or lost card)."""
    if "admin" not in session:
        return jsonify({"status": "error", "message": "Unauthorized"}), 401

    data = request.json
    emp_id = int(data.get("employee_id"))
    time_out_str = data.get("time_out")  # "HH:MM" 24hr
...
# =====================================================
# NEW FEATURE 4: FORCE SIGN-OUT (stop button)
# =====================================================
@app.route("/api/force_signout/<int:record_id>", methods=["POST"])
def force_signout(record_id):
    """Admin force-closes an open attendance record right now."""
    if "admin" not in session:
        return jsonify({"status": "error", "message": "Unauthorized"}), 401
...
# (respects break time + pay window via compute_paid_minutes)
# =====================================================
def update_salary_background():
    while True:
        ...
        for record in active_records:
            ...
            cursor.execute("""
                UPDATE opti_rec SET duration=%s, salary=%s WHERE id=%s
